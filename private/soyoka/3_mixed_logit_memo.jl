...
parameter = Parameter(theta2 = [0.001, 0.001, 0.001])
@unpack Nsim, T, N, theta2 = parameter

vec_draw = randn(Nsim*size(matrix_x2, 2))
market_index = vec_draw[1:Nsim*length(theta2)]
...
parameter = Parameter(Nsim = 500, T = T, N = N, theta2 = [0.001, 0.001, 0.001])
...
f_mktshare(parameter, vec(delta))
...
while norm > tol && itr < max_itr
  ...
  # R code: exp_delta <- exp_delta_old * share_obs / pred_mkt_share expで計算しているのでupdateの式がテキストと異なる
...
output = transpose(Xi)*Z*W*transpose(Z)*Xi
